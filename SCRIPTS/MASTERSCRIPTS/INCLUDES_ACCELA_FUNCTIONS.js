...
function addFeeByDate(feeCap, fdate, fcode, fsched, fperiod, fqty, finvoice) // Adds a single fee, optional argument: fCap
{
	...
	if (!f) {
		logDebug("could not instantiate RefFeeBusiness to determine fee version, exiting addFeeByDate");
		return false;
	}
	...
	if (!vDate) {
		logDebug("could not convert date parameter to javascript date, exiting addFeeByDate");
		return false;
	}
	...
	if (!v) {
		logDebug("could not determine default version for fee schedule " + fsched + ", exiting addFeeByDate");
		return false;
	}
...
function capSet(desiredSetId) {
	...
	this.add = function (addCapId) {
		...
		if (setMemberStatus)
			this.updateMemberStatus(addCapId, setMemberStatus);
...
function copyCalcVal(fromcap, newcap) {
	...
	if (!newcap) {
		logMessage("**WARNING: copyCalcVal was passed a null new cap ID");
		return false;
	}
...
function countIdenticalInspections() {
	...
	if (arguments.length > 0)
		oldDateStr = arguments[0]; // Option to override olddate in the parameter
		...
		for (xx in inspList) {
			if (String(inspType).equals(inspList[xx].getInspectionType()) && String(inspResult).equals(inspList[xx].getInspectionStatus())) {
				if (convertDate(inspList[xx].getInspectionStatusDate()) < oldDate) {
					if (!oldInspectionFound) {
						cntResult++;
						oldInspectionFound = true
					}
				} else {
					cntResult++
				}
...
function createPublicUserFromContact() // optional: Contact Type, default Applicant
{
	...
	var userModel;
	if (arguments.length > 0)
		contactType = arguments[0]; // use contact type specified
...
function deleteTask(targetCapId, deleteTaskName) {
	...
	if (!tTask) {
		logDebug("**WARNING: Task not found: " + deleteTaskName);
		return false;
	}
...
function editBuildingCount(numBuild) // option CapId
{
	...
	cdWrite = aa.cap.editCapDetail(cd)
		...
		if (cdWrite.getSuccess()) {
			logDebug("Updated building count to " + numBuild);
			return true;
		} else {
			logDebug("**ERROR writing capdetail : " + cdWrite.getErrorMessage());
			return false;
...
function editCreatedBy(nCreatedBy) {
	...
	if (!setCreatedByResult.getSuccess()) {
		logDebug("**WARNING: error setting cap created by : " + setCreatedByResult.getErrorMessage());
		return false
	}
...
function editHouseCount(numHouse) // option CapId
{
	...
	cdWrite = aa.cap.editCapDetail(cd)
		...
		if (cdWrite.getSuccess()) {
			logDebug("Updated house count to " + numHouse);
			return true;
		} else {
			logDebug("**ERROR writing capdetail : " + cdWrite.getErrorMessage());
			return false;
...
function editInspectionRequiredFlag(inspType, reqFlag) {
	...
	if (!result.getSuccess()) {
		logDebug("**ERROR retrieving inspection milestones: " + result.getErrorMessage());
		return false;
	}
	...
	if (!inspMilestones) {
		logDebug("No Inspection Milestones found");
		return false;
	}
...
function editPriority(priority) // option CapId
{
	...
	cdWrite = aa.cap.editCapDetail(cd)
		...
		if (cdWrite.getSuccess()) {
			logDebug("updated priority to " + priority);
			return true;
		} else {
			logDebug("**ERROR writing capdetail : " + cdWrite.getErrorMessage());
			return false;
...
function editRefLicProfAttribute(pLicNum, pAttributeName, pNewAttributeValue) {
	...
	licObj = getRefLicenseProf(pLicNum)
		...
		if (!licObj) {
			logDebug("**WARNING Licensed Professional : " + pLicNum + " not found");
			return false
		}
	...
	if (licSeqNum == null || attributeType == "" || attributeType == null) {
		logDebug("**WARNING Licensed Professional Sequence Number or Attribute Type missing");
		return false
	}
	...
	if (!peopAttrResult.getSuccess()) {
		logDebug("**WARNING retrieving reference license professional attribute: " + peopAttrResult.getErrorMessage());
		return false
	}
...
function editReportedChannel(reportedChannel) // option CapId
{
	...
	if (cdWrite.getSuccess()) {
		logDebug("updated reported channel to " + reportedChannel);
		return true;
	} else {
		logDebug("**ERROR writing capdetail : " + cdWrite.getErrorMessage());
		return false;
	...
	for (i in wfObj) {
		...
		if (fTask.getTaskDescription().toUpperCase().equals(wfstr.toUpperCase()) && (!useProcess || fTask.getProcessCode().equals(processName))) {
			...
			if (tResult.getSuccess())
				...
				logMessage("**ERROR: Failed to update task, comment visible flags on workflow task: " + tResult.getErrorMessage());
				return false;
			}
		}
	}
...
function executeASITable(tableArray) {
	...
	for (xx in tableArray) {
		...
		var myCriteria = doTableObj["Criteria"];
		aa.print("cri: " + myCriteria)
		var myAction = doTableObj["Action"];
		aa.print("act: " + myAction)
...
//
{
	...
	if (retArr.length > 1) {
		logDebug("**WARNING: Multiple caps returned for this address/apptype");
		return retArr[0]
	}
...
function getChildren(pCapType, pParentCapId) {
	...
	var childArray = getCapResult.getOutput();
	if (!childArray.length) {
		logDebug("**WARNING: getChildren function found no children");
		return null;
	}
...
// doWarning = true, message if license is expired.
{
	...
	if (getout.getSuccess())
		...
		logDebug("**ERROR: communicating with CSLB: " + getout.getErrorMessage());
		return false;
	}
...
function getInspectionScheduleParams4Notification(params) {
	...
	if (InspectorFirstName)
		addParameter(params, "$$InspectorFirstName$$", InspectorFirstName);

	if (InspectorMiddleName)
		addParameter(params, "$$InspectorMiddleName$$", InspectorMiddleName);

	if (InspectorLastName)
		addParameter(params, "$$InspectorLastName$$", InspectorLastName);
...
function getUserFullName() {
	...
	if (systemUserObjResult.getSuccess()) {
		...
		if (!matches(systemUserObj.getFirstName(), null, undefined, ""))
			userFullName = systemUserObj.getFirstName();
		if (!matches(systemUserObj.getLastName(), null, undefined, ""))
			userFullName += " " + systemUserObj.getLastName();
			...
			for (var j = 0; j < guideItemASITs.size(); j++) {
				...
				for (var k = 0; k < columnList.size(); k++) {
					...
					while (iteValues.hasNext()) {
						...
						if (tableArr[zeroBasedRowIndex] == null)
							tableArr[zeroBasedRowIndex] = new Array();
...
function insertTask(sourceTaskName, newTaskName, insertTaskType) {
	...
	if (taskResult.getSuccess()) {
		...
		aa.print("ERROR: Failed to insert task! Path = " + taskPath + ";" + taskResult.getErrorMessage());
		return false;
	}
...
function isRenewProcess(parentCapID, partialCapID) {
	...
	if (parentCapID == null || partialCapID == null) {
		logDebug("ERROR: the parentCapID or the partialCap ID is null");
		return false;
	}
...
{
	itemCap = capId;
	if (!matches(vCapId, undefined, null, ""))
	...
	this.licType = vLicType // Licence Type (optional)
		...
		if (b1ExpResult.getSuccess()) {
			...
			logDebug("(licenseObject) **ERROR: Getting B1Expiration Object for Cap.  Reason is: " + b1ExpResult.getErrorType() + ":" + b1ExpResult.getErrorMessage());
			return false
		}
...

{
	...
	if (!contact) {
		logDebug("Couldn't link public user for " + contactType + ", no such contact");
		return false;
	}

	if (contact.getPeople().getContactTypeFlag().equals("organization")) {
		logDebug("Couldn't link public user for " + contactType + ", the contact is an organization");
		return false;
	}
	...
	if (getUserResult.getSuccess() && getUserResult.getOutput()) {
		...
		logDebug("Couldn't link public user for " + contactType + ", no such public user");
		return false;
...
function loadTasks(ltcapidstr) {
	...
	for (i in wfObj) {
		...
		if (fTask.getStatusDate())
			myTask.statusdate = "" + (fTask.getStatusDate().getMonth() + 1) + "/" + fTask.getStatusDate().getDate() + "/" + (fTask.getStatusDate().getYear() + 1900);
		...
		for (var i in workArr) {
			...
			if (workVals[0] > capval)
				...
				if (valNumber == 2)
					...
					if ((capval - workVals[0]) % divisor > 0)
						saveVal++;
					saveVal = saveVal * workVals[valNumber];
				}
		}
...
function prepareRenewal() {
	...
	if (isRenewProcess(parentCapId, capId)) {
		...
		logDebug("Renewal Process did not finish properly");
		return false;
	}
}
...
function validateGisObjects() {
	...
	for (a1 in fGisObj) // for each GIS object on the Cap
	{
		...
		if (gischk.getSuccess())
			...
			logDebug("**WARNING: Retrieving GIS Attributes.  Reason is: " + gischk.getErrorType() + ":" + gischk.getErrorMessage());
			return false
		}
