function activateTask(wfstr) // optional process name
{
	...
	for (i in wfObj) {
		...
		if (fTask.getTaskDescription().toUpperCase().equals(wfstr.toUpperCase()) && (!useProcess || fTask.getProcessCode().equals(processName))) {
			...
			if (useProcess)
				aa.workflow.adjustTask(capId, stepnumber, processID, "Y", "N", null, null)else
...
function addASITable(tableName, tableValueArray) // optional capId
{
	...
	var fld = tsm.getTableField()
		...
		for (thisrow in tableValueArray) {
			...
			if (tsm.setReadonlyField)
				tsm.setReadonlyField(null); // check for 6.6.1.   If so need to populate with null
...
function convertStringToPhone(theString) {
	...
	for (var x = 0; x < compString.length; x++) {
		if (compString[x] >= "A" && compString[x] <= "Z")
			retString += n[compString.charCodeAt(x) - 65]else
...
function createPublicUserFromContact() // optional: Contact Type, default Applicant
{
	...
	var getUserResult = aa.publicUser.getPublicUserByEmail(contact.getEmail())
		if (getUserResult.getSuccess()) {
			...
			if (userModel)
				return userModel; // send back the existing user
		}
...
function deactivateTask(wfstr) // optional process name
{
	...
	for (i in wfObj) {
		...
		if (fTask.getTaskDescription().toUpperCase().equals(wfstr.toUpperCase()) && (!useProcess || fTask.getProcessCode().equals(processName))) {
			...
			if (useProcess)
				aa.workflow.adjustTask(capId, stepnumber, processID, "N", completeFlag, null, null)else
...
function getGISInfo(svc, layer, attributename) {
	...
	if (gisObjResult.getSuccess())
		...
		logDebug("**ERROR: Getting GIS objects for Cap.  Reason is: " + gisObjResult.getErrorType() + ":" + gisObjResult.getErrorMessage());
		return false
	}
...
function licenseObject(licnumber) {
	...
	for (myStatus in licenseStatus) {
		...
		if (b1ExpResult.getSuccess()) {
			...
			logDebug("**ERROR: Getting B1Expiration Object for Cap.  Reason is: " + b1ExpResult.getErrorType() + ":" + gisObjResult.getErrorMessage());
			return false
		}
	}
...
function loadTasks(ltcapidstr) {
	...
	for (i in wfObj) {
		...
		if (fTask.getStatusDate())
			myTask.statusdate = "" + fTask.getStatusDate().getMonth() + "/" + fTask.getStatusDate().getDate() + "/" + (fTask.getStatusDate().getYear() + 1900);
	...
	if (foundID) {
		resultResult = aa.inspection.resultInspection(itemCap, foundID, inspStatus, resultDate, resultComment, currentUserID)
			...
			if (resultResult.getSuccess())
				logDebug("Successfully resulted inspection: " + inspType + " to Status: " + inspStatus)else
...
function updateWorkDesc(newWorkDes) // optional CapId
{
	...
	if (workDesScriptObj)
		workDesObj = workDesScriptObj.getCapWorkDesModel()else {
...
function validateGisObjects() {
	...
	for (a1 in fGisObj) // for each GIS object on the Cap
	{
		...
		if (gischk.getSuccess())
			...
			logDebug("**ERROR: Retrieving GIS Attributes.  Reason is: " + gischk.getErrorType() + ":" + gischk.getErrorMessage());
			return false
		}
