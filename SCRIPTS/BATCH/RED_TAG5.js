/*------------------------------------------------------------------------------------------------------/
| Program: ActivePaidRedCases.js  Trigger: Batch    Client : Clearwater, FL
|
| Version 1.0 - Base Version. 11/2/2010 - Nicholas Dorrough - City of Clearwater
|
| Move cases from Red to Active when a red fee is paid
|
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
...
var CommentNote = "Updated Via Batch: Active Paid Red Cases";
...
if (bizDomScriptResult.getSuccess())
{
	...
	else
	{
		do
		{
			...
			if( entry != null )
			{
				doActivateCase( entry.getDispBizdomainValue() );
			}
		}
...
function doActivateCase( altid )
{
	...
	logDebug( "Moving " + altid + " from Red to Active." );
	updateAppStatus("Active",CommentNote);
}
